function y = applyAirAbsorptionMC(x,fs,fc)
% function assumes that the time sample is explicitly gives the distance
% travelled. This multichannel (MC) version expects a multichannel signal
% whose time domain length is in dimension 1.
...
Lx = size(x,1); % signal length
Sz_x = size(x); % input dimensions
Sz_mc = prod(Sz_x(2:end)); % multichannel dimension sizes combined
Sz_x_rshp = [Lx prod(Sz_x(2:end))];
x = reshape(x, Sz_x_rshp); % compress MC dimensions into 1
Sz_x_pad = Sz_x;
x_padding = zeros(winspan+L-1, Sz_mc); % padding for head and tail of signal

% get input signal length and pad beginning and end
% x = [zeros(winspan+L-1,1); x; zeros(winspan+L-1,1)];
% signal_length = length(x);
x = cat(1, x_padding, x, x_padding); % pad head and tail of signal
...
y = zeros([signal_length + fftsize, Sz_mc]);     % allocate output+'ringing' vector
...
%     SIG = SIG(1:specsize,:); % Only need the first half           
    pad_sig = cat(1, win .* x(i:(i+winspan-1), :), zeros(fftsize-winspan, Sz_mc));
    SIG = fft(pad_sig, [], 1);  
    SIG = SIG(1:specsize, :); % Only need the first half           
