function trajectory = B_LISTENER_TRAJECTORY(scene, GENERATE_ANIM)
...
if GENERATE_ANIM
    ...
    p_vid = subdivideTrajectory(cPts, framedist);
    ...
    v = VideoWriter([scene.path_out filesep 'orchestra_moving.avi'], 'MPEG-4');
    open(v);
...
trajectory.rir_pts = subdivideTrajectory(cPts, trajectory.rir_dist);
...
trajectory.intrp_pts = subdivideTrajectory(cPts, framedist);
% compute interpolation weights for every segment point in the trajectory
trajectory.intrp_weights = getPlanarTriInterpWeights(scene.rec, trajectory.intrp_pts);
